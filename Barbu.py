...
class Joueur:
    ...
    def __init__(self, id, risque, type):
        ...
        self.texte_premier = None
        self.texte_points = None
...
def jouer():
    ...
    if not Pli :
        C.itemconfig(Liste_joueurs[joueur_courant].texte_premier,state=tk.NORMAL)
    ...
    if joueur_courant == NUM_UTILISATEUR:
        ...
        if carte:
            ...
            if carte in cartes_jouables :
                C.itemconfig(Label_annonce, text='')
                del(Liste_joueurs[joueur_courant].main[Liste_joueurs[joueur_courant].main.index(carte)])
                nb_carte_jouee += 1
                joueur_courant = (joueur_courant+1)%4
            else :
                C.itemconfig(Label_annonce,text='Carte invalide : \nJouez un {x}'.format(x=Pli[0].couleur.lower()))
    ...
    if len(Pli) == 4 :
        ...
        val_pli = determiner_points_pli()
        C.itemconfig(Label_annonce,text='Joueur {x} perd \n + {y} points !'.format(x=joueur_courant+1,y=val_pli))


        Liste_joueurs[joueur_courant].points += val_pli
        for joueur in Liste_joueurs :
            C.itemconfig(joueur.texte_premier,state=tk.HIDDEN)
            C.itemconfig(joueur.texte_points,text='{x} Pts'.format(x=joueur.points))
        del Pli
        Pli = []
        premiere_carte = None
...
def manche_suivante():
    ...
    if manche <= NB_MANCHES:
        C.itemconfig(Label_annonce,text='MANCHE {x}'.format(x=manche))
        jouer()
    else :
        fin()
...
def creer_liste_joueurs():
    ...
    Liste_joueurs[0].texte_premier = C.create_text(498, 345, text='Premier', font=('Impact', 13), fill='#6D1616')
    Liste_joueurs[0].texte_points = C.create_text(498, 750, text='0 pts', font=('Impact', 20), fill='#6D1616')

    Liste_joueurs[1].texte_premier = C.create_text(260,190, text='Premier', font=('Impact', 13), fill='#6D1616')
    Liste_joueurs[1].texte_points = C.create_text(260,380, text='0 pts', font=('Impact', 13))

    Liste_joueurs[2].texte_premier = C.create_text(498, 40, text='Premier', font=('Impact', 13), fill='#6D1616')
    Liste_joueurs[2].texte_points = C.create_text(498, 230, text='0 pts', font=('Impact', 13))

    Liste_joueurs[3].texte_premier = C.create_text(740, 190, text='Premier', font=('Impact', 13), fill='#6D1616')
    Liste_joueurs[3].texte_points = C.create_text(740, 380, text='0 pts', font=('Impact', 13))
...
def afficher_regles():
    ...
    msg = tk.Message(top, text="Le joueur avec le plus de points perd.\n\n"
                               "Il faut (si possible) jouer une carte de la même couleur (pique...) que celle du premier à avoir joué\n\n"
                               "Chaque Pli (ensemble de 4 cartes vaut des points en fonction de la manche).\n\n"
                               "On \"remporte\" le Pli en ayant joué la carte la plus haute de la même couleur que celle imposée.\n\n"
                               "Points en fonction de la manche : \n"
                               "\t-Manche 1 : +4 par Coeur présent.\n"
                               "\t-Manche 2 : 4 par Pli.\n"
                               "\t-Manche 3 : +16 par Reine présente.\n"
                               "\t-Manche 4 : +40 pour le Roi de Coeur.\n"
                               "\t-Manche 5 : Toutes les règles précédentes.\n\n"
                               "Manche actuelle : {x}".format(x=manche))
    msg.pack()

    button = tk.Button(top, text="Ok", width = 5, command=top.destroy)
    button.pack()

def selec_carte(evt):
    global carte_selectionnee
    if evt.x <= 163 and evt.x >= 58 and evt.y <= 85 and evt.y >= 40:
        afficher_regles()
    else :
...
Label_annonce = C.create_text(500,285,text='', font=('Impact',20))

C.create_text(265, 284, text='2', font=('Impact',35))
C.create_text(500, 137, text='3', font=('Impact',35))
C.create_text(741, 284, text='4', font=('Impact',35))
...
C.create_text(110,60,text='Regles',font=('Impact',25))
C.create_rectangle(58, 40, 163, 85,width=2)
